		...
		RongCallClient.getInstance().setVoIPCallListener(new IRongCallListener() {
			...
			public void onCallDisconnected(RongCallSession callProfile, RongCallCommon.CallDisconnectedReason reason) {
				...
				if ( SingleCallActivity.erunnable==null) {
					...
					SingleCallActivity.ehandler.removeCallbacks(SingleCallActivity.erunnable);
				...
				int  type = SharedPreferencesUtil.getInt(mContext, "chattype", 0);
				if (type==0) {
					SharedPreferencesUtil.putInt(mContext, "chattype", 1);
				}else {
